...
public:
	...
	~GLWidget();
	void loadData(QVector<Point3D> PntsList);									// 读取数据
	void getBoxNum(double &BoxLength, double &BoxWidth, double &BoxHeight);		// 得到外框数据
	void getCurrentPnt(int& PntNum);											// 得到当前显示点数
...
private:
	...
	QVector<Point3D>		m_ListPnts;					// 存储mainwindow过来的点
	QVector<Point3D>		LBPntsVec;					// 存储滤波后的点
	double					m_MoveIntoScreen = 0;		// 移入屏幕的距离
